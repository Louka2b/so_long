...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/01/27 18:03:17 by marvin            #+#    #+#             */
/*   Updated: 2026/01/27 18:03:17 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************** ************************************ */
...
{
    map->collec_img = mlx_xpm_file_to_image(mlx, "asset/collectible.xpm", &width, &height);
    if (!map->collec_img)
    {
    ...
    map->exit_img = mlx_xpm_file_to_image(mlx, "asset/exit.xpm", &width, &height);
    if (!map->exit_img)
    {
...
void ft_load_images(void *mlx, t_map *map)
{
    ...
    width = 0;
    height = 0;
    map->floor_img = mlx_xpm_file_to_image(mlx, "asset/floor.xpm", &width, &height);
    if (!map->floor_img)
    {
    ...
    map->wall_img = mlx_xpm_file_to_image(mlx, "asset/wall.xpm", &width, &height);
    if (!map->wall_img)
    {
...
void ft_draw_map(void *mlx, void *mlx_win, t_map *map)
{
    ...
    while (y < map->y)
    {
        ...
        {
            if (map->map[y][x] == '0')
                mlx_put_image_to_window(mlx, mlx_win, map->floor_img, x * tile_size, y * tile_size);
            else if (map->map[y][x] == '1')
                mlx_put_image_to_window(mlx, mlx_win, map->wall_img, x * tile_size, y * tile_size);
            else if (map->map[y][x] == 'C')
                mlx_put_image_to_window(mlx, mlx_win, map->collec_img, x * tile_size, y * tile_size);
